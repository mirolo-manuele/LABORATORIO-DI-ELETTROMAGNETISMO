...
df = pd.read_csv("/Users/alessiobrusini/Desktop/LABORATORIO-DI-ELETTROMAGNETISMO/relazioni/esperienza_esame/codici/file_csv/WaveData592.csv", sep=",", comment='#')
df1 = pd.read_csv("/Users/alessiobrusini/Desktop/LABORATORIO-DI-ELETTROMAGNETISMO/relazioni/esperienza_esame/codici/file_csv/WaveData597.csv", sep=",", comment='#')
df2 = pd.read_csv("/Users/alessiobrusini/Desktop/LABORATORIO-DI-ELETTROMAGNETISMO/relazioni/esperienza_esame/codici/file_csv/WaveData598.csv", sep=",", comment='#')
n_soglia_inf = 0
n_soglia_sup = 2500
R_in= 693
C_in = 0.00047
...
t_np = df.iloc[n_soglia_inf+25:n_soglia_sup+25, 0].to_numpy(dtype=float)
V_np = df.iloc[n_soglia_inf+25:n_soglia_sup+25, 1].to_numpy(dtype=float)
# Estrai i dati come array NumPy
t_np1 = df1.iloc[n_soglia_inf:n_soglia_sup, 0].to_numpy(dtype=float)
V_np1 = df1.iloc[n_soglia_inf:n_soglia_sup, 1].to_numpy(dtype=float)

t_np2 = df2.iloc[n_soglia_inf+125:n_soglia_sup+125, 0].to_numpy(dtype=float)
V_np2 = df2.iloc[n_soglia_inf+125:n_soglia_sup+125, 1].to_numpy(dtype=float)
...
mg = ROOT.TMultiGraph()
mg.SetTitle("Confronto tra due CSVt [s]; Voltaggio [V]") # Titolo globale;X;Y
...
leg.AddEntry(g, "V_in", "lp")
leg.AddEntry(g2, "R = 693 #Omega", "lp")
leg.AddEntry(g3, "C = 470 #muF", "lp")
leg.Draw()
    # Manteniamo la finestra aperta (necessario se esegui da script)
c.Update()
print("V_medio_maggiore =", np.mean(V_np2))
print("V_ripple_maggiore =", (np.max(V_np2)-np.min(V_np2)))
print("fattore di Ripple =", (np.max(V_np2)-np.min(V_np2))/np.mean(V_np2))
print("Abbassamento tensione =", max(V_np)-max(V_np1))

c.SaveAs("ponte_confronto.pdf")