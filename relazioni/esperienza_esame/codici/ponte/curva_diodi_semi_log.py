...
for j in fogli_da_processare:
    ...
    df = df.dropna(subset=["V[V]","ln_I[mA]"])

    V = array('d', (df["V[V]"].to_numpy(dtype=float) )) #convert i mV in V
    lnI = array('d', df["ln_I[mA]"].to_numpy(dtype=float))
...
c.Modified() 
c.Update()
c.SaveAs("curva_diodi_semilog.pdf") 
