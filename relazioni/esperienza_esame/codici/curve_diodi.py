...
for j in range(1,5): #la funzione range prende crea un intervallo chiuso a sx e aperto a dx
    #nelle prossime righe faccio leggere tutti i dati da tutti i fogli(tolgo i NaN)
    s=str(j)
    foglio="Diodo_"+s
    df = pd.read_excel("curva_diodi.xlsx", sheet_name=foglio)
    df = df.dropna(subset=["V[V]","I [mA]"])

    V = array('d', df["V[V]"].to_numpy(dtype=float))
    lnI = array('d', df["I [mA]"].to_numpy(dtype=float))

    eV = array('d', df["err_V"].to_numpy(dtype=float))
    elnI = array('d', df["err_I"].to_numpy(dtype=float))
...
dummy_graph.SetTitle("Curva caratteristica diodi;V[V]; I[mA]") 
...
for j in range(1,5):
    ...
    foglio="Diodo_"+s
    df = pd.read_excel("curva_diodi.xlsx", sheet_name=foglio) #legge il file excel
    df = df.dropna(subset=["V[V]","I [mA]"]) #controlla solo le colonne V e I. Se una di queste contiene un valore mancante, elimina quella riga

    V = array('d', df["V[V]"].to_numpy(dtype=float)) #df è il dataframe, "V" è il nome della colonna da leggere 
    lnI = array('d', df["I [mA]"].to_numpy(dtype=float))  #to_numpy converte la colonna in un array NumPy di tipo float.
