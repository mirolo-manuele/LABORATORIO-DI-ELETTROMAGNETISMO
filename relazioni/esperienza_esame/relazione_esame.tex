...
Per inidcare la bontà delle osservazioni si è tenuto conto di due fattori:
\begin{itemize}
...
Nel caso nostrocaso possaimo osservare che:
\begin{itemize}
    \item nel grafico \ref{fig:curva_ponte_2} ho $r=0.095$ nel caso di $C= 470 \: \mu F$;
    \item nella conifguazione con diodi inverti (grafico \ref{fig:curva_ponte_invertito}) ho $r=0.076$ nel caso di $C= 470 \: \mu F$;
    \item nel grafico \ref{fig:curva_ponte_3} ho $r=0.051$ avendo una resistenza maggiore;
\end{itemize}
Nel confronto tra il \ref{fig:curva_ponte_2} e l' \ref{fig:curva_ponte_invertito} possiamo osservare una differenza del coefficiente di ripple.
Questo è dovuto alla differenza di tensione media che si può osservare nei grafici. La differenza di tensione media può essere attribuita ad un differente comportamento del condensatore od ad un errata presa dati.
Si esclude l'influsso del cambiamento del diodo, in quanto nella figura \ref{fig:curva_diodi} si osserva che la differenza a parità di corrente è dell'ordine di $ 20 mV$, troppo piccola per spiegare la nostra differenza di tensioni medie, che si attesta su:
