...
df = pd.read_excel("faraday.xlsx", sheet_name="Dati filtrati")
...
fit = ROOT.TF1("fit", "pol1", min(V), max(V)) # intervallo del fit, è scontato che prende minore e maggiore valore di x
fit.SetParName(0, "q")
fit.SetParName(1, "m")
...
em = fit.GetParError(1)
eq = fit.GetParError(0)
