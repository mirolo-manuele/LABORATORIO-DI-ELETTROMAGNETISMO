...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{presa_dati_manuale}
    \caption{andamento della tensione a capi del capacitorecd}
    \label{fig:esempio}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{fit_presa_dati_manuale}
    \caption{grafico in scala logaritmica}
    \label{fig:esempio}
\end{figure}
 
Questi due grafici rappresentano l'andamento della tensione nella sua forma esponenizale e nella sua espressione in scala logaritmica. In questo caso i dati sono stati presi manualemnte per osservare e verificare che effettivmente l'andamento sperimentale rispecchi almeno in parte quello teorico.
È possibile osservare sia dal primo, ma soprattutto dal secondo grafico che nella fase di carica si riscontra l'effettivo andamento lineare, mentre dagli istanti di tempo successivi a $ 6 \dot 10^{-6}$ risulta esserci una deviazione dalla linearità, questoprobabilmente è  causato dal raggiungimento della tensione di regime la cuiazione di quest'ultima si avvicina a zero:
...
$\frac{V_0Q_s}{\sqrt{2}}$. In questo modo si ottiene L=5,83 mH
