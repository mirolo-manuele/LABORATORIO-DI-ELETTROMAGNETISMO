...
g=ROOT.TGraphErrors(len(Vsqrt), Vsqrt, I,)
...
indici_fit = [i for i in range(len(Vsqrt)) if Vsqrt[i] >= v_soglia and Vsqrt[i]<= v_min]

x_fit = array('d', [Vsqrt[i] for i in indici_fit])
