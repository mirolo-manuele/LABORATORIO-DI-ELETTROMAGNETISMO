...
E = h \nu \quad \nu = \frac{c}{\lambda}
\]
se l'energia del fotone è maggiore della banda proibita (massima energia che l'eletrone ha per essere legato all'atomo), allora l'elettrone può essere eccitato nella banda di conduzione. 
Nel caso il diodo sia polarizzato da una tensione inversa, l'elettrone eccitato può attraversare la giunzione p-n, creando una corrente di \textit{fotocorrente}.
...
\\
Nel caso di polazizzazione diretta (diminuzione della zona di svuotamento), la funzione d'onda dell'elettrone ha una probabilità maggiore di sovrapporsi con la funzione d'onda della lacuna, emettendo un fotone (LED).
...
Se allora il fotodiodo lo alimento con una tensione negativa (polarizzazione inversa) posso misurare la corrente di fotocorrente che dipende dalla frequenza della luce incidente.
...
\[
F = e \cdot NA = 9.64853321233100184 \times 104 \; C/mol.
\]

Per misurate questa costante si usa:
\begin{itemize}
    \item bacinella di acqua distillata con discolto del solfato di rame ($CuSO_4$) \\
    ...
    Questo mi porta a studiare il mio sistema come se fosse delle sfere che si muovo in un fluido puro (acqua), allora entra in gioco la forza di Stokes: $F= 6 \pi \eta r v$.
    \item inseriamo due elettrodi di rame collegati ad un generatore di tensione continua ed un amperometro in serie. 
    Quando accendiamo il generatore sull'elettrodo negativo (catodo) si avvicinano gli ioni $Cu^{2+}$ che catturando 2 elettroni poi si legano all'elettrodo stesso; processo simile avviene all'anodo con lo ione $SO_4^{2-}$ che cedendo i due elettroni forma di nuovo il sale che si stacca dall'elettrodo e si dissolve nell'acqua. 
    ...
    \[
    \frac{\Delta m}{MM_{Cu}} N_A 2 e = Q = \sum_{i=1}^{n} I(t_i) \Delta t 
    \]
    ...
    \item da queste considerazioni ottengo dunque:
    \[F= \frac{MM_{Cu}}{2 m} \sum_{i=1}^{n} I(t_i) \Delta t \]
    se misuro circa gli stessi intervalli di tempo posso far uscire il mio $\Delta t$. 
    \item a causa delle variazioni di temperatura della concentrazioni devo fare una soluzione \textit{sovrasatura};
\end{itemize}
