...
\subsubsection{Fase di misurazione}
Questo strumento, in generale, viene utilizzato per misurare delle tensioni che variano nel tempo, attraverso il posizionamento 
di un condensatore piano collegato al generatore di tensione variabile (che genera il segnale da studiare) e posizionato con le due faccie sopra e sotto il fascio di elettroni. 

\begin{center}
\begin{tikzpicture}[
    >=Stealth,
    electron/.style={circle, fill=red, minimum size=2pt},
    plate/.style={fill=gray!30, minimum width=3cm, minimum height=0.3cm},
    gun/.style={fill=gray!20},
    grid/.style={fill=gray!40, pattern=north east lines, pattern color=gray}
]

% Dimensioni principali
\def\L{10}   % Lunghezza totale
\def\H{6}    % Altezza totale
\def\d{1.5}  % Distanza tra le piastre
\def\l{2}    % Lunghezza piastre

% CANNONE ELETTRONICO (sinistra)
\begin{scope}[xshift=1cm, yshift={\H/2}]
    % Filamento riscaldante
    \draw[thick, decorate, decoration={coil, amplitude=3pt, segment length=3pt}] 
        (0,0.3) -- (0,-0.3);
    
    % Catodo
    \draw[gun] (-0.4,-0.6) rectangle (0,0.6);
    \fill[black] (-0.2,0) circle (2pt);
    ...
    \foreach \y in {-0.2,0,0.2} {
        \node[electron] at (1.2,\y) {};
        \draw[red, dashed, ->] (1.2,\y) -- (2,\y);
    }
    
    % Etichette cannocchiale
    \node[above] at (0.5,0.8) {elettronico};
    \node[above] at (0.5,1.1) {Cannone};
\end{scope}

% GRIGLIA DI CONTROLLO
\draw[grid] (3,-0.2) rectangle (3.3,0.4);
\draw[black, thick] (3.15,-0.2) -- (3.15,-0.6);
% FASCIO ELETTRONICO PRIMA DELLE PIASTRE
\draw[red, thick, ->] (3,1.4) -- (4.5,1.4) node[midway, above] {Fascio elettronico};

% CONDENSATORE PIANO PER DEFLESSIONE
\begin{scope}[xshift=5cm, yshift={\H/2}]
    % Piastra superiore
    \node[plate] (upper) at (0,\d/2+0.15) {};
    \draw[->, thick] (upper.south) -- ++(0,-0.3) node[right, pos=0.5] {$+$};
    
    % Piastra inferiore  
    \node[plate] (lower) at (0,-\d/2-0.15) {};
    \draw[->, thick] (lower.north) -- ++(0,0.3) node[right, pos=0.5] {$-$};
    
    % Campo elettrico
    \foreach \x in {-0.8,-0.4,0,0.4,0.8} {
        \draw[blue, ->, thick] (\x,-\d/2+0.2) -- (\x,\d/2-0.2);
    }
    
    % Elettrone che entra
    \node[electron] at (-1.2,0) {};
    \draw[red, dashed, ->] (-1.2,0) -- (-0.5,0);
    
    % Elettrone che esce deviato
    \draw[red, dashed, ->] (1,0.1) to[out=20, in=180] (3,1.5);
    \draw[black, thick] (-1.5,-1.5) -- (1.5,-1.5) node [midway, below] {l};
    \draw[black, thick] (-4.5,2) -- (4.5,3) node [midway, below] {L};
    \draw[black, thick] (2.5,-0.8) -- (2.5, 0.8) node [midway, left] {d};
\end{scope}
    
    \draw[fill=green!20, very thick] (9,-1.5) rectangle (9.2,2);
\foreach \y in {0.5,1.5,-1.3,-0.5} {
    \draw[green!50!black, fill=green!30] (9.1,\y) circle (2pt);
}
\draw[black, thick] (8.8,0.2) -- (8.8,1.8) node[midway, above] {D};
...
% Griglia
\foreach \x in {0,1,...,4} \draw[gray!20] (\x,-1.2) -- (\x,1.2);
\foreach \y in {-1,0,1} \draw[gray!20] (0,\y) -- (4.5,\y);

% Tre sinusoidi sfasate
\draw[red, thick, domain=0:4, samples=100] plot (\x, {sin(360*\x)});
\draw[blue, thick, domain=0:4, samples=100] plot (\x, {sin(360*\x + 120)});
\draw[green, thick, domain=0:4, samples=100] plot (\x, {sin(360*\x + 240)});
...
\begin{figure}[h]
    \centering
        \begin{tikzpicture}
            \begin{axis}[
                title={Andamento ad onda quadra di V(t)},
                xlabel={t},
                ylabel={V(t)},
                xmin=0, xmax=4,
                ymin=-1.5, ymax=1.5,
            ...
            % Definizione dell'onda quadra
            \addplot [blue, thick, domain=0:4] { 
                mod(floor(x),2) == 0 ? 1 : -1
            };
...
\begin{center}
    \begin{circuitikz}[european, scale=1.2]
        \draw (0,0) 
            to [I, l=$G$, name=gen] (0,3) % Generatore di corrente variabile
            to [R, l=$R$, name=res] (3,3) % Resistenza
            to [C, l=$C$, name=cap] (3,0) % Condensatore
            to [short] (0,0); % Chiusura del circuito
...
\begin{figure}[h!]
    \centering
        \begin{tikzpicture}
        \begin{axis}[
            title={Carica di un condensatore: $Q = Q_0(1-e^{-\frac{t}{RC}})$},
            xlabel={Tempo $t$},
            ylabel={Carica $Q$},
            xmin=0, xmax=5,
            ymin=0, ymax=1.2,
            ...
            extra x tick style={grid style={red,dashed}},
        ]

        % Funzione di carica del condensatore
        \addplot [blue, thick, domain=0:5] {1 - exp(-x)};
        \node at (axis cs:3,0.7) {$Q = Q_0(1-e^{-t/RC})$};

    
        \addplot [red, dashed, domain=0:5] {1};
        \node[red] at (axis cs:4,1.1) {$Q = Q_0$};
...
\begin{center}
    ...
    \draw (0,0) 
        to [V, l=$V$, i=$I$] (0,3)           % Generatore
        to [R, l=$R$, i=$I$] (3,3)           % Resistenza
        to [short, i=$I$] (4,3)
        to [C, l=$C$, v=$V_C$] (4,0)         % Condensatore
        to [short] (0,0);
        
    % Lampadina in parallelo
    \draw (3,3) 
        to [lamp, l=Lampada, i=$I_L$] (3,0); % Lampadina
    \end{circuitikz}
\end{center}
...
\section{Lezione 06/11/2025}
\subsection{Circuiti RCL}
Immaginiamo un sistema che emette radiazioni elettromagnetiche, potremmo considerarlo isolato se e solo se consideriamo anche lo spazio attorno ad esso e per un periodo di tempo breve.
Le leggi di Kirkhoff, come già detto, sono valide solo istante per istante, dunque rappresentano delle approssimazioni della realtà.\\
Questo dunque genera della dispersione di nostri parametri che sono dovute all' energia che viene assorbita od emessa dal sistema.

Ricordiamo che un qualunque tratto di circuito è caratterizzato da un' \textit{induttanza}:
ogni circuito costituito da resistenza è assimilabile da un circuito con un'induttanza (in serie) e una capacità (in parallelo)\textit{parassita}, 
ovvero trascurabili rispetto a un resistenza posta antecedentemente. Ciò cambia in base al comportamente induttivo o capacitivo del circuito; per indicare queste caratterisitche si usa una 
lettera sovrastata da un asterisco $L^* \; C^*$.\\ 

Stimolando il circuito (con resistenza in serie ad un capacitore) con un'onda quadra e analizzando il segnale con un oscilloscopio, in base a come abbiamo inserito capacità e resistenza, le curve caratteristiche potrebbero apparire
con oscillazzioni all'inizio e alla fie della curva, ciò è indicativo della presenza di un induttanza in parallelo. 

Creiamo un circuito con 

\begin{itemize}
    \item generatore di tensione variabile
    \item in serie ad una resistenza $V=IR$
    \item in serie ad un induttanza a cui capi vale $V=L\frac{dI}{dt}$
    \item in serie ad un condensatore per cui vale $V=\frac{Q}{C}$
\end{itemize}

\begin{center}
    \begin{circuitikz}[american]
        % Circuito con generatore, resistenza, capacità e induttanza in serie
        \draw (0,0) to[V, invert, l=$V(t)$] (0,2) 
                    to[R] (2,2)
                    to[C, name=myC] (4,2)  % Condensatore
                    to[L] (6,2)            % Induttanza dopo il condensatore
                    to[short] (6,0)
                    to[short] (0,0);
        
        % Oscilloscopio in parallelo al condensatore
        \draw (2,2) to[short, *-] (2,3) 
                    to[oscope, l=Oscilloscopio] (4,3)
                    to[short, -*] (4,2);
    \end{circuitikz}
\end{center}
dunque dovrebbe valere
\[
V(t)=V_R+V_L+V_C=IR + L \frac{dI}{dt}+\frac{Q}{C} = \underbrace{\frac{dQ}{dt}R}_{\text{smorzante}}+ L \frac{d^{2} Q}{dt^{2}}+\frac{Q}{C}
\]
che è un equazione differenziale del secondo ordine, in particolare è simile a quella del moto armonico smorzato.

Studiando ora il dominio dei tempi, dunque come il sistema evolve nel tempo dopo essere stato perturbato. Studiando un impulso d'energia semplice, che rimane costante nel tempo,
ciò vuol dire c
\begin{equation*}
    V(t)=
    \begin{cases}
    ...
    \end{cases}
\end{equation*}
Risolvendo agli autovalori ottengo:
\[
\lambda = -\frac{\Gamma}{2} \pm \underbrace{\sqrt{\frac{\Gamma^2}{4}- {\omega_0}^2}}_{\Delta}
\]
dove  $\Gamma = \frac{R}{L}$ e $\omega_0 = \frac{1}{\sqrt{CL}}$.

Abbiamo diversi casi:
\begin{itemize}
    \item $\Delta =0$: smorzamento semplice
    \item $\Delta >0$: sovrasmorzamento, soluzione esponenziale decrescente
    \item $\Delta <0$: andamento oscillante smorzato, il cui andamento analitico è
    \[
       Q(t)=Q_0 \exp(-\frac{\Gamma}{2} t ) \cos(\omega t)
    \]
\end{itemize}
il termine (esponenziale) di smorzamento è in funzione della resisteza e dell'induttanza, proporzionale a $R$ e inversamente a $L$.

\subsection{Misura di $L$}
Misureremo la d.d.p. ai capi del capacitore, utilizzando un oscilloscopio (lavoro con frequenze elevate), per la tensione ai capi delle piastre so che:
\[
V(t)=\frac{Q(t)}{C}=\frac{Q_0}{C} \exp(-\frac{\Gamma}{2} t ) \cos(\omega t)
\]
(i dati di $C$ ed $R$ sono noti dal costruttore)

Dalla legge ricavata posiamo ricavare $L$, poichè la frequenza dell'oscillazione smorzata è quella intrinseca del circuito. 
All'inizio dell'oscillazione ho un andamento dovuto a quello di una onda quadra (in realtà trapezoidale, smussata sugli spigoli), 
dunque imprecisa per i calcoli (contributi sistematici ignoti a causa della non quadriticità dell'onda e della sua pendenza).

Ricaveremo $\omega_0=\frac{2 \pi}{T}$, da cui dalla formula precedente otterremo $L$.

\begin{itemize}
    \item i 3 nodi da cui ricavo il periodo li ricavo dallo 0 del voltaggio, per individualizzarli imposto l'oscilloscopio su ground ($0V$), che costringe lo strumento
    ad oscillare attorno allo $0$;
    \item a causa dello smorzamento non apprezzerò più un oscillazione, ma una linea continua, essa rappresenta gli $0$ Volt. Devo verificare che essa coincida con il ground dell'oscilloscopio.
    \item Per fare ciò devo far si che il semiperiodo dell'onda quadra debba essere t.c.: $\frac{2\pi T}{2} >> \frac{1}{\omega_0}$;
    \item La linea che passa per i massimi (almeno $5/6$) nei massimi del segnale deve essere esponenziale $\exp(-\frac{\Gamma}{2}t)$ (svolgo un fit in scala semigolaritmica $\left(\log(V(t))= \log (V_0)-\frac{\Gamma}{2}t\right)$);
    \item se ribalto ora i punti con ascissa negativa, dovrei trovare degli altri punti che sono contenuti nella curva esponenziale. La somma dei massimi e minimi (in scala semilogaritmica) risulta $-\frac{\Gamma}{2}$;
    \item L'errore maggiore introdotto è dato da una lettura sbagliata dello $0$ del voltaggio (poichè ciò cambia il centro d'oscillazione).
    \item Se il periodo non è troppo diverso $\left(\text{ovvero se} \quad \omega_0 \simeq \frac{2 \pi}{T}\right)$ dal periodo di oscillazione del circuito (il quale dipende solo da caratterisitche intrinseche del circuito $\omega_0 = \frac{1}{\sqrt{CL}}$)
allora la differenza fra i massimi e minimi dell'oscillazione e del segnale risulta trascurabile. 
    \item Per determinare il valore del periodo dell'onda quadra, dovrò iniziare la misura modificando la frequenza dell'onda, e notare quando ciò modifica l'oscillazione dello smorzamento. Faremo durare l'onda quadra 8 volte il periodo totale dato dalle 3 oscillaziooni, 
in modo da diminuire l'errore. 
    \item L'opportuna terna di $RCL$ è determianta dal fatto che i rumori esterni al circuito non influiscano il segnale, bisognerà scegliere ampiezza maggiori di $3V$. ($\nu \in [0,\text{qualche migliaio} \quad kHz]$)
\end{itemize}

\subsection{Onda a dente di sega: $2^\circ$ parte }
Rimembriamo il circuito della lampadina a Neon, 

\begin{center}
    \begin{circuitikz}[american]
    \draw (0,0) 
        to [V, invert, l=$V$, i=$I$] (0,3)           % Generatore
        to [R, l=$R$, i=$I$] (3,3)           % Resistenza
        to [short, i=$I$] (4,3)
        to [lamp] (4,0)         % Condensatore
        to [short] (0,0);
        
    % Lampadina in parallelo
    \draw (3,3) 
        to [C, name=myC] (3,0); % Lampadina
        
    % Oscilloscopio in parallelo alla lampadina
    \draw (3,3) to [short, *-] (5,3) 
                to [oscope, l=Oscilloscopio] (5,0)
                to [short, -*] (3,0);
    \end{circuitikz}
\end{center}

Bisogna inanzitutto utilizzare dei valori di $RC$ consoni; e vedere poi come vario il periodo del dente di sega, la variabile su cui ci si può giostrare meglio è la tensione con cui sto alimentando il circuito. 

Ciò perchè la lampadina si comporta come un condensatore di capacità $C^*$ e in parallelo ad un altro capacitore: $C_{eq} = C + C^*$. Dunque preso $V_0^*>V_0$, la curva diventerà più ripida,
poichè nello stesso tempo dovrà caricare la stessa percentuale del voltaggio massimo $V_0$. 

Salendo dunque col voltaggio $V_0$ si può osservare se c'è una collerazione fra $\nu$ e $V_0$. Per spiegarne il risultato dovrei prevedere a priori l'andamento, ma dobbiamo considerare
\begin{itemize}
    \item la polarizzazzione degli atomi
    \item la cessione dell'energia da parte degli atomi a causa degli urti
\end{itemize}

\section{Lezione 14/11/2025}
\subsection{Circuiti RC in serie}
Dal circuito mostrato in figura otterremo, dalla legge di Kirkhoff
\begin{equation}
V(t)=V_C+V_R=\frac{Q}{C}+R \frac{dQ}{dt}
\label{voltage}
\end{equation}
...
\begin{circuitikz}
    ...
    \node at (4,2) [above right] {3};
\end{circuitikz}
\end{center}
Se la soluzione di questa equazione differenziale è del tipo $V(t)=\cos (\omega t)$, considereremo l'estenzione complessa:
\[
V(t)=V_0 \exp (i\omega t)
\]
Ricordando che il significato fisico risiede nella parte reale.
Poichè $V$ e $I$ sono legati a meno di una costante troveremo le stesse soluzioni per $I(t)$:
\[
I(t)=I_0 \cos (\omega t) \rightarrow I(t)=I_0 \exp (i\omega t)
\]
e sostituendo nella \eqref{voltage} otteniamo:
\[
V(t)=I(R+\frac{1}{i \omega C}) 
\]
da cui si ottiene facilemnte la parte reale:
\begin{align*}
I(t) &= \text{Re}[I(t)] == \text{Re}\left[\frac{\vec{V}  e^{j\omega t}}{R + 1/(j\omega C)}\right] = \text{Re}\left[\frac{\omega C\vec{V}  e^{j\omega t}}{-j + \omega RC}\right] = \\
&= \text{Re}\left[\frac{\omega C\vec{V}  e^{j\omega t}e^{j\pi/2}(1 - j\omega RC)}{1 + (\omega RC)^2}\right] = \quad \text{ponendo} \quad \omega_0 = \frac{1}{RC} \\
&= \text{Re}\left[\frac{\vec{V}  e^{j\left[\omega t + \pi/2 - \arctan(\omega/\omega_0)\right]} \omega/(R\omega_0)}{\sqrt{1 + (\omega/\omega_0)^2}}\right] = \text{Re}\left[\frac{\vec{V}  e^{j\left[\omega t + \arctan(\omega_0/\omega)\right]}}{R\sqrt{1 + (\omega/\omega_0)^2}}\right] = \\
&= \frac{V}{R\sqrt{1 + (\omega_0/\omega)^2}} \cos\left(\omega t + \arctan\frac{\omega_0}{\omega}\right) = I  \cos(\omega t + \varphi_i)
\end{align*}
da cui notiamo che $I(t)$ precede $V(t)$ di un angolo $\varphi_i = \arctan(\omega_0/\omega)$, inoltre avremo $0 \leq I \leq \frac{V}{R}= I_{max}$ come aspettato.
...
Consideriamo la risposta del circuito RLC, dato da un onda quadra, allora i valori consigliati da inserire sono:
\begin{itemize}
    \item R = 100 $\Omega$
    \item L = 40 mH
    \item C = 0.07 $\mu F$
...
Avrò un errore dovuto alla determinazione del massimo(e alle oscillazioni intorno ad esso), allora per dedurre il periodo misuro la distanza tra tre modi consecutivi
 quando intercano l'asse dei tempi. 
\\
Introduciamo allora un fattore di qualità $Q_s = \frac{\omega_0}{\Gamma}$, le mie tensioni reali ai capi delle mie componenti generate da un segnale di stimolo del tipo $V(t)= V_0 \sin(\omega t)$ sono:
\[
V_R= \frac{V_0}{\sqrt{1 + A^2}} \cos(\omega t - A)\]\[
V_L = \frac{\omega}{\omega_0} \frac{V_0 Q_s}{\sqrt{1 + A^2}} \cos(\omega t - A + \frac{\pi}{2})\]\[
V_C= \frac{\omega_0}{\omega} \frac{V_0}{\sqrt{1 + A^2}} \cos(\omega t - A - \frac{\pi}{2})
\]

dove $ A = Q_s (\frac{\omega}{\omega_0}- \frac{\omega_0}{\omega})$

Ai capi dell'induttanza mi aspetto un valore $V=V_0 Q_S$, ma per quanto detto, i valori della tensione graficati danno una curva il cui massimo è proprio ha valore  $V=V_0 Q_S$,
e sulle ascisse troverem il valore proprio del circuit $\omega=\frac{1}{\sqrt{RC}}$.

Presa la larghezza della curva  alla quota $\frac{V_0 Q_s}{\sqrt{2}}$, trovo che essa è identica a $\Gamma$, dunque aumentando la resistenza mi aspetto un circuito in cui la curva rimane sostanzialmente la stessa ma $\Gamma$ aumenta.\\
Dunque per una misura appropriata dovrò aumentare le frequenze per campionare i valori di $V(t)$ che trovo. 


La potenza emessa dal circuito risulta $W \propto \Gamma^2$, però in questo caso il valore di $\Gamma$ è la \textbf{larghezza a metà ampiezza}.  

Dunque noto che al variare della frequenza del segnale di stimolo cambia la tensione ai capi dell'induttanza, se però voglio distinguere due segnali differenti
voglio ottenere una curva a picco (per esempio diminuendo la resistenza o aumentando l'induttanza), in modo che l'interferenza fra i due sia la minore possibile (sto attuando una
\textit{modulazione in frequenza}), chiameremo $\Gamma$ la \textit{banda di ricezione} mentre $Q_S$ è \textit{l'efficenza della modulazione}.
\\ 
Si noti che modificando $C$ vado a variare $\omega_0$, dunque la mia frequenza che voglio ricevere (la manovella della radio). 

Lo sfasamento temporale dell'onda si valuta andando a far coincidere l'asse del segnale principale con quello misurato ai capi dell'induttanza.

In condizioni di risonanza lo sfasamento sara (per quanto visto prima)
\begin{itemize}
    \item 0 su $V_R$
    \item $\pi / 2$ su $V_C$ e $V_L$
\end{itemize}
I problemi potrebbero essee:
\begin{itemize}
    \item rumori esterni
    \item la geometria interne delle scatolette d'induttanza
\end{itemize}

\section{Lezione 20/11/2025}
\subsection{Effetto Hold}
Immaginiamo che presa una piastra conduttrice, omogenea e parallelepipedea, sul piano cartesiano e che vi sia una corrente $I$ verso l'asse $y$, mentre vi sia
un campo magnetico $\vec B$ verso l'asse $z$.
Dal punto di vista elettrico:
\[
I=\vec{J} A \hat n = J l c \quad J= n e v
\]
dove $\vec J$ è la densità di corrente, possiamo passare ai moduli perchè abbiamo tutti vettori ortogonali ($l$, $c$ sono lato e spessore del rettangolo che forma il parallelepipedo)
$v$ è la velocità della corrente e $n$ ed $e$ sono rispettivamente LA densità di carica degli elettroni e la loro carica.

A causa di B agisce sugli elettroni la Forza di Lorentz:
\[
F_{mag}=e \vec v \times \vec B = evB
\]
nel nostro caso considero solo i moduli essendo corrente e campo magnetico ortogonali. 

\begin{center}
\begin{tikzpicture}[scale=1, every node/.style={font=\small}]
    % 3D box (parallelepipedo) via offset
    \def\dx{0.8}
    \def\dy{0.5}
    \draw[thick] (0,0) rectangle (3,2); % front face
    \draw[thick] (\dx,\dy) rectangle (\dx+3,\dy+2); % back face (offset)
    \draw[thick] (0,0) -- (\dx,\dy);
    \draw[thick] (3,0) -- (\dx+3,\dy);
    \draw[thick] (0,2) -- (\dx,\dy+2);
    \draw[thick] (3,2) -- (\dx+3,\dy+2);

    % corrente I (lungo y, verso l'alto)
    \foreach \x in {0.6,1.5,2.4}{
        \draw[->, thick, blue] (\x,0.15) -- (\x,1.85);
    }
    \node[blue, right] at (3.15,1) {$\vec I\;(\hat{y})$};

    % campo magnetico B (perpendicolare, rappresentato da punti "verso di noi")
    \foreach \xx in {0.9,1.5,2.1}{
        \foreach \yy in {0.4,1.0,1.6}{
            \node at (\dx+\xx,\dy+\yy) {$\odot$};
        }
    }
    \node[above] at (\dx+1.5,\dy+2.2) {$\vec B\;(\hat{z})$};

    % forza di Lorentz su un elettrone: v along +y, B along +z => v x B = +x, q=-e => F = -e (v x B) = -x
    \draw[->, red, thick] (1.5,1) .. controls (1.0,0.8) .. (0.35,0.55) node[left] {$\vec F_{mag}$};
    \node[red,left] at (0.25,0.5) {$e^-$};

    % accumulo cariche sulle facce (segno convenzionale dovuto alla deviazione)
    \node at (3.35,0.35) {+};
    \node at (3.35,0.95) {+};
    \node at (3.35,1.55) {+};
    \node at (-0.25,0.35) {-};
    \node at (-0.25,0.95) {-};
    \node at (-0.25,1.55) {-};
    \node[below] at (1.5,-0.2) {Accumulazione di cariche laterali};

    % assi di riferimento
    \draw[->] (0,-1.0) -- (2.5,-1.0) node[right] {$\hat{x}$};
    \draw[->] (-1.0,0) -- (-1.0,2.2) node[above] {$\hat{y}$};
    \draw[->] (4.5,0) -- (4.5,1.2) node[above] {$\hat{z}$};
\end{tikzpicture}
\end{center}

La forza (tramite la regola della mano destra) e ricordando che l'elettrone ha carica negativa, fa compiere all'elettrone una traiettoria curva.
Al di sotto di questa carica, dunqe, rimangono cariche postive, creando una $\Delta V$, dunque un campo $\vec E$, che contrasta la forza di Lorenzt:
\[
F_{el}=\frac{e V_H}{a}
\]
dove $a$ è la distanza fra i lati delle piastre del parallelepipedo e $V_H$ il potenziale di Hold:
\[
\frac{V_H}{a}= vB
\]
da cui otteniamo:
...
I=nveac
\]
ricavo allora il potenziale di Hold pari a: 
\[
\frac{V_H}{a}= \frac{BI}{naec} \rightarrow V_H = \frac{BI}{nec}
\]
quindi fra due facce opposte rispetto alla direzione della corrente si instaura un d.d.p. prorzionale a $B$ ed $I$ e inversamente proporzionale allo spessore ($c$), che posso misurare
grazie ad un voltmetro (ai capi delle facce orizzontali) e un amperometro (ai capi delle facce ortogonali). \\
Questo mi permette di trovare la \textbf{densità di portatori di carica} in un determinato materiale conduttore. 
...
Risulteranno valori molto bassi per $V_H$, per dimiuire l'errore il parametro più gestibile è $c$, comunque limitato (rischio di fondere la piastrina.)

\subsection{Errori sistematici}
\begin{itemize}
    \item i puntali che misurano $V_H$ devono esspere sullo stesso asse, altrimenti si risente di una resistenza $R \propto \rho \text{e} c$ dove $\rho$ è la resistività del materiale.
    \item bisogna misurare la d.d.p. senza $B$, per evitare di misurare come $V_H$ valori di altre cadute di potenziale
    \item La corrente va misurato in entrambi in sensi
\end{itemize}

\subsection{Misura di $\frac{e}{m}$}
Prendiamo un pallone di vetro che contiene un cannoncino elettronico, riempito con $Ne$.
Utilizzando \textit{bobine di Helmotz} 
\footnote {ovvero 2 bobine identiche, distanti in come il loro diametro (per rendere più piccolo $B_{centro}/B_{ovunque}$), e paralleli ad un asse passante per il centro. Voglio creare un volume
con $B$ uniforme nello spazio fra le due bobine. Fatto ciò possiamo immaginare che le linee di campo siano ortogonali alle bobine e parallele fra loro. }

Possiamo dunque immaginare che la bolla sia immerso in $B$ uniforme, all'interno abbiamo una bassissima pressione, producendo il fascetto collimato di elettroni che sbatterà contro
il vetro, essendo l'energia degli elettroni prodotti dal cannoncino $\simeq 100 eV$
\[
\rightarrow
\]
Avendo inserito il $Ne$ (e non essendo dunque nel vuoto) gli elettroni incontrano atomi di $Ne$, rilasciando energia cinetica ed eccitandoli. $Ne$ emette fotoni nel visibile, dunque posso vedere
il fascio di elettroni. L'importante è che la pressione da far si che l'elettrone, nonostante gli urti, non abbia perso troppa $E_{cin}$.\\
Ora applicando il campo uniforme delle bobine di Helmontz avrò su ogni elettrone la forza di Lorenz che mi devia l'elettrone,
 dando luogo ad una forza centripeta, facendo seguire all'elettrone un arco di cerchio (ciò accade, in realtà, solo se $B$ è perfettamente ortogonnale e $e^-$ non perde $E$).\\
Fatte talune approssimazioni:
\[
evB =m_e\frac{v^2}{r} \rightarrow eB= m_e \frac{v}{r}
\]
dove $r$ è il raggio dell'arco di cerchio. Allora ottengo:
\[
\frac{e}{m} \propto \frac{V}{r^2 B^2}
\]
dunque se misuro in modo indipendente $m$ ed $e$ posso ricavare l'uno dal'altro.
Gli altri valori risultano:
\begin{itemize}
    \item $V=V_H = 1V$
    \item $r$: sapendo che la diversità dall'orbita circolare si nota solo dopo metà della circonferenza (infatti risulta essere una scia), 
    posso definire dunque un punto a meno dello spessore della scia, costruendo una scala coincidente col diametro della circonferenza percorsa, posso far coincidere la traccia luminosa
    (ovvero quando una tacca copre la traccia, evitando errori di parallasse) con la circonferenza. luminosa. \\
    Sapendo la distanza di tale tacca conosciamo il diametro della circonferenza 
    (a meno di un errore di $\frac{1}{4}mm$, cioè lo spessore della tacca, ma esso è compatibile con l'errore della misura)
    \item Nel cannoncino elettronico: Dati $V_c$ tensione di collimazione e $V_a$ la d.d.p. all'elettrodo accellerante. Possiamo ricavare gli effetti di bordo (quando l'elettrone è quasi fuori dalla zona di collimazione)
    Tali effetti si ripercuotono sull'elettrone in uscita (che sente la tensione negativa dell'elettrodo di collimazione): $V_{eff} < V_a$, ma possiamo dire che $V_a > V_{eff} > V_a - V_c$, amche se sarà più vicino al primo estremo .\\
    Qusto può cambiare molto la misura, sottostimando $\frac{e}{m}$
    \item Campi elettrici del cannoncino: hanno effetti sulla traiettoria, soprattutto se $r$ è molto piccolo 
    \item Il pezzo di vetro in cui schiantano gli elettroni si carica negativamente: costruisco un grafico $I_{fascio} $ (I=intensità), quindi tali parti di vetro tendono a respingere gli $e^-$, duqnue si riduce il raggio,
    il quale ha un contributo di $\frac{1}{r^2}$ alla formula 
\end{itemize}

\subsection{Misura di 0}
Abbiamo un cannoncino elettronico che colpisce uno schermo che si illumina, se non vi sono altri campi vedremo il punto perfettamente nel centro dello schermo.\\
Inserendo una coppia di placchette deflettrici con potenziale $V$, possiamo misurare la distanza $d$ dal centro dello schermo (dovuta alla traiettoria parabolica assunta durante il passaggio fra le placchette).\\
Immaginiamo di poter creare $B$ uniforme agente solo fra le placchette (con bobine di Helmontz), orientato ortogonalmente al campo elettrico $E$ delle placchette. Dunque abbiamo forza di Lorentz:
\[
F_{mag}=\frac{eV}{d} + evB 
\]
con $v$ ricavata tramite l'energia cinetica ($eV_e = \frac{1}{2}m_ev^2 $), mettendomi nella condizione:
\[
 (E=\frac{V}{d})-vB=0
\]
l'elettrone sente su di se forza nulla dunque, riscrivendo $ v= \sqrt{\frac{2eV_e}{m_e}}$..
Questo  mi permette di poter ricavarre il rapporto carica su massa, in una maniera diistinata dal caso precedente. 

Per sapere di aver bilanciato $E$ basta guardande che il segnale sia ritornato al centro dello schermo

\begin{itemize}
    \item a causa di campi esterni non avrò un singolo spot luminoso ma una circonferenza, oppure di altra forma (più difficile per determinare il centro)
    \item Se il campo magnetico $B$ non agisce solamente sul volumetto occupato dalle due piastre delefftrici, avrò un errore. Sapendo che 
    $B$ ha la stessa direzione, è sufficente dunque un campo più piccolo rispetto a quello teorico, nelle equazioni otteniamo una sovracorrezione. Poiche $\frac{e}{m} \propto \frac{1}{B^2}$ 
    sto sovrastimando il rapporto

\end{itemize}

\section{Lezione 01/12/2025}
\subsection{Misura del campo magnetico locale}
Nel laboratorio che solitamente utilizziamo ci sono campi magnetici dovuti alla presenza di molto materiale ferro-magnetico nello scantinato e magneti negli armadi. Il nostro scopo è quello di non avere campi magnetici variabili.\\

Immaginiamo di essere in un ambiente dove vi è un campo magnetico ambientale costante, la cui direzione e verso è quella della "diagonale" della stanza. Il campo magnetico terrestre infatti ha componente radiale e parallela (al piano che ci troviamo)
Se chiamiamo $\vec{m}$ il momento di dipoolo della terra, allora avremo che il campo magnetico prima citato sarà $B_{rad}= \frac{\mu_0}{4\pi}\frac{2m \cos(\theta)}{R_T^3}$ e $B_{par}= \frac{\mu_0}{4\pi}\frac{2m \sin(\theta)}{R_T^3}$.    
Utilizzando una bobina, incardianata perpendicolarmente al suo asse, che ruota tramite una manovella, possiami far cambiare il flusso di $B$ al ruotare di essa seconndo $\sin (\omega_0t)$ con $\omega_0$ e la velocità angolare costante.\\
Dunque cambia $V$, possiamo ricavare $B_{terrestre}$:
\[
V(t)=n \omega_0(t) \underbrace{\pi r^2}_{\text{area bobina}} B_{locale} \sin (\phi) \sin (\theta) \sin (\omega_0t)
\]
Se l'asse di rotazione della bobina si trova nel piano verticale contenente il campo magnetico della stanza, il flusso in tal caso è minore (infatti se l'asse di rotazione avesse direzione parallela a quello del campo magnetico, il flusso sarebbe nullo)

\begin{itemize}
    \item n numero di spire
    \item $\omega(t)$ è la velocità angolare
    \item 
\end{itemize}

Facciò ciò per diversi angoli, passando in condizioni di massimo e minimo per il campo magnetico. In questo modo possiamo capire modulo e direzione del campo magnetico. Per valutare il verso colleghiamo la bobina 
ad un oscilloscopio, potremmo vedere un andamento sinusoidale (dovuto a $\omega_0$ costante), conoscendo la \textit{legge di Lentz} (un circuiti si oppone alla variazione del campo amgnetico al suo interno), dall'incipit della rotazione, se il primo lobo è negativo
Posso ricavare $T$ dal diagramma dell'oscilloscopio (da cui si trova $\omega_0$), succesivamente definisco gli angoli $\theta$ e $\phi$ e scelgo i tempi in modo che $\sin (\omega_0t)$ abbia valore unitario. Da qui posso valutare il verso di $B$.\\

Alcune osservazioni
\begin{itemize}
    \item Poichè la bobina deve ruotare il filo viene collegato all'esterno tramite collegamento strisciante con l'oscilloscopio (inizialmente non dovrò ricavare dati causa la presenza di ossidi).
    \item Le sinusoidi che otterremo nell'oscilloscopio avranno frequenza dell'ordine di $6/7 Hz$, da cui ottengo un massimo di $15mV$, dell'ordine del rumore ambientale. Inserisco un filtro \textit{passabasso}, un circuito RCL
    che filtra tutte le frequenze più alte di $\simeq 10Hz$ (ovvero le frequenze disturbanti verrano abbassate d'ampiezza) 
    \item insereremo inoltre un amplificatore per le nostre frequenze, in modo che il rumore residuo sia compatibile con lo spessore della traccia luminosa. Dovremo dunque calibrare l'amplificatore (per capire il valore dell'amplificazione).
    Una volta tarato potremmo inserire la bobina, conoscendo valore di filtraggio e amplificazione.
    \item l'errore atteso su $B$ è sulla seconda cifra decimale
\end{itemize}

\subsection{Creazione di un raddrizzatore}
La corrente viene trasportata con la tensione alternata, se ho bisogno di una tensione continua devo raddrizzarlo.

Se applico un circuito composto da resistenza e diodo, con la corrente che polarizza direttamente quest'ultimo mi aspetto che, misurando il voltaggio ai capi del diodo, trovo un segnale "a orecchie di gatto"(quando il diodo è polarizzato inversamente
non dovrebbe esserci passaggio di corrente, dunque non dovrei vedere alcun segnale \footnote[1]{in realtà una piccola corrente passa lo stesso ma è trascurabile}), la media di $V$ sul diodo mi fornisce il segnale di raddrizzamento. 

Voglio stiracchiare il segnale per non avere voltaggi negativi, per farlo inserisco in parallelo al diodo e resistenza un condensatore, se il filo utilizzato ha bassa resistenza la corrente troverà una direzione privilegiata, poichè 
salita del segnale e caricamento del condensatore avvengono allo stesso tempo, quando il segnale si abbassa il condensatore si scarica e riemette la carica nel circuito con la mia resistenza. 

La resistenza totale del circuto sarà $R^* = R' +R$ con $R'$ resistenza del filo che collega il circuito al condensatore, e l'andamento del voltaggio sarà $V(t) = V_0 \exp (-t/R^*C)$
\begin{center}
\begin{circuitikz}[american] % Usa lo stile americano (resistenze a zigzag)
    
    % --- Ramo 1: La serie Resistenza + Diodo ---
    % Disegniamo una linea orizzontale da (0,0) a (6,0)
    % I terminali 'o-o' indicano i nodi di ingresso/uscita aperti
    \draw (0,0) to[R, l=$R$, o-] (3,0) 
                to[D, l=$D$, -o] (6,0);

    % --- Ramo 2: Il Condensatore in parallelo ---
    % Partiamo dal nodo iniziale (0,0), saliamo, mettiamo il condensatore e scendiamo alla fine (6,0)
    \draw (0,0) -- (0,2) 
                to[C, l=$C$] (6,2) 
                -- (6,0);

    % --- Etichette dei Nodi (Opzionale) ---
    \node[left] at (0,0) {A};
    \node[right] at (6,0) {B};
...
Otterrò una corrente quasi costante caratterizzata dalla \textit{tensione di Ripple} (differenza fra massimi e minimi):
\[
\Delta V_r= V_{\max}- V_{\min}
\]
Potrò dire di avere un sistema che raddrizza il segnale minore è tale valore. Considero il valore della tensione:
\[
<V> = \frac{1}{2}  V_{\max} + V_{\min}
\]
Ovviamente avremo $<V> < V_0$, così come per la corrente. 

L'ampiezza che non passa per il condensatore va nel diodo "buttandolo via", per recuperare questa corrente 
\begin{center}
% Utilizziamo lo stile americano per i componenti e le tensioni
\begin{circuitikz}[american, node distance=2.5cm]

    % --- Definizione dei nodi principali del ponte ---
    % Immagina un rombo. Definiamo i 4 vertici.
    \coordinate (AC_left) at (0, 2.5);   % Ingresso AC sinistro
    \coordinate (AC_right) at (5, 2.5);  % Ingresso AC destro
    \coordinate (DC_plus) at (2.5, 5);   % Uscita DC positiva (alto)
    \coordinate (DC_minus) at (2.5, 0);  % Uscita DC negativa (basso)

    % --- Disegno dei 4 Diodi del Ponte ---
    % Nota: D* disegna un diodo pieno (nero), spesso preferito negli schemi.
    % L'orientamento è cruciale per il funzionamento del ponte.
    
    % Diodo in alto a sinistra (punta verso il +)
    \draw (AC_left) to[D*, l=$D_1$, *-*] (DC_plus);
    % Diodo in alto a destra (punta verso il +)
    \draw (AC_right) to[D*, l=$D_2$, *-*] (DC_plus);
    % Diodo in basso a sinistra (punta via dal -)
    \draw (DC_minus) to[D*, l=$D_3$, *-*] (AC_left);
    % Diodo in basso a destra (punta via dal -)
    \draw (DC_minus) to[D*, l=$D_4$, *-*] (AC_right);


    % --- Sezione di Ingresso (Sorgente AC) ---
    % Disegniamo una sorgente sinusoidale a sinistra
    \draw (AC_left) -- (-2, 2.5) 
          to[sV, l=$V_{in} (AC)$] (-2, 0) % sV = sinusoidal Voltage source
          -- (DC_minus); % Connessione al nodo inferiore


    % --- Sezione di Uscita (Carico Resistivo) ---
    % Disegniamo una resistenza a destra per rappresentare il carico
    \draw (DC_plus) -- (7, 5) 
          to[R, l=$R_{Load}$, v=$V_{out} (DC)$] (7, 0) 
          -- (DC_minus);

    % --- Aggiunta di etichette di polarità (Opzionale) ---
    % Aggiungiamo un + e un - per chiarire l'uscita DC
    \node[above] at (DC_plus) {+};
    \node[below] at (DC_minus) {-};

\end{circuitikz}
\end{center}


Tramite questi circuiti posso recuperare i lobi dei segnali di negativi, essi vengono chiamati \textit{ponti di Graetz}, mi è necessaria una capacità circa la metà di quella di prima per ottenere la stessa
tensione di ripple, col vantaggio di avere una tensione più costante rispetto a prima.

Abbiamo inserito nel circuito un \textit{trasformatore}, il quale trasforma la tensione d'ingresso in ampiezza, tramite un accopiamento di bobine.\\

Per prima cosa dovremo verificarre che le curve caratteristiche dei quattro diodi si assomiglino. Al variare dei parametri del circuito ($C$ e $R$) varia il comportamento del segnale:
\begin{itemize}
 \item la forma del segnale di corrente inversa deve essere più dolce possibile
 \item la resistenza del diodo cambia bruscamente all'inizio e a lla fine delle orecchie di gatto, il segnale reale infatti ivi risulta più ripida del segnale teorico (a causa del cambiamento esponenziale della resistenza del diodo)
 ivi perdiamo un po' di potenza che può essere valutata
\end{itemize}


\subsection{Misura del campo magnetico prodotto da un filo}
Supponendo un filo infinitamente esteso vogliamo trovare l'andamento del campo magnetico, per farlo, insereremo due staffe in modo da tendere il filo il più possibile (anchè se avraà una forma catenaria) e attaccate ai lati di un tavolo
La \textit{freccia} (differenza fra il minimo della catenaria e la retta passante per i due estremi superiori delle staffe) deve essere trascurabile rispetto alla distanza fra gli estremi del filo.\\

Mi aspetto che il filo rispetti la relazione:
\[
B_* \propto \frac{I}{r}
\]
Colleghiamo gli estremi del filo ad un alimentatore, esso (i collegamenti) produrrà un campo magnetico, ma sarà trascurabile a quello in prossimità del filo. La condizione ($R << D$) ci garantisce la bontà della misura. 

Orientando il tavolo parallelo alla componente parallela del campo magnetico locale, e così anche per il filo, mi aspetto un campo magnetico ortgonale al filo, dunque 
il campo totale sarà la somma di $B_{\parallel}$ e $B_{\perp}$, inserendo una bussola, essa avrà orientazione eguale a quella di $B_{tot}$. 

Misuando l'angolo fra $B_{\parallel}$ e $B_{tot}$, e prendendone la tangente posso associare questa al variare della corrente, dopodichè farò lo stesso ma variando la distanza. 
Le maggiori cause di errore sistematico sono:
\begin{itemize}
    \item la distanza fra gli estremi deve essere molto maggiore di quella  fra bussola e filo, in modo da avere l'approsimazione di un filo infinitamente estesom, se non lo sono, sottostimo il valore di $B$
    \item se inserisco il fulcro dell'ago pari al raggio dove effettuo la misura su tutto l'ago è diverso (agli estremi dell'ago il campo è più piccolo), sto sottostimando il campo
\end{itemize}

\section{Lezione 04/12/2025}
\subsection{Misura di Millikan}

Si tratta della prima misura della carica elementare. Poichè era impossibile per l'epoca avere microscopi molto raffinati, lo sperimentatore creò un oggetto costituito da:
\begin{itemize}
    \item 2 placche metalliche separate 
    \item un foro al di sopra di una delle armature 
    \item al suo interno inseriamo un \textit{tubo di venturi}, questo serve a nebulizzare un liquido.
    \item un vetro con cui guardare le goccioline nebulizzate tramite un microscopio.
    \item sul lato laterale inseriamo una luce che serve per far "brillare le goccioline"
\end{itemize}
In tale modo possiamo definire la posizione della particella che diffonde la luce, 
in realtà si osserveranno delle lineette, a causa delle lenti convergenti
del microscopio, le quali rovesciano l'immagine che si vuole osservare.
Dunque le goccioline superano un reticolo costiutiiti da tacche, sapendo la distanza fra le tacche
posso dedurre la velocità di salita della gocciolina. \\

\subsubsection{Effetto di Stokes}
L'effetto della forza di gravità, poichè, sapendo che una mole di acqua si trova in $22.4L$ di aria, possiamo supporre,
data la dimensione dello strumento, che le goccioline siano più grandi delle particelle con cui urtano, 
dunque è possibile utiilizzare la legge di Stokes.\\
Possiamo approssimare la forma della gocciolina ad una sfera oscillante, se utilizziamo un liquido molto denso (nel nostro caso olio di paraffina), 
la sua forma rimarrà più stabile, causa la maggior intensità richiesta alle forze elastiche per deformarla. \\
Data tale apprssimazione posso applicare la legge di Stokes, che formulata assume la seguente forma:
\[
\vec F_{\text{Stokes}}=6\pi \eta  R \vec{v}
\]
dove $\eta$ è la viscosità dell'aria, il quale dipende da vari fattori, quali la temperatura. 
La componente della velocità d'interesse è quella perpendicolare al piano, la risultante delle forze e':
\[
F_{tot}=-mg + F_{stokes} = -mg + 6\pi \eta  R \vec{v}
\]
La velocità si modifica nel tempo, all'equilibrio (a $t=\infty$) le due forze si bilanciano, ma 
l'equazione differenziale associata è ideale, il quale vale solo se $2R$ è relativamente grande rispetto
al cammino libero medio (senza fare urti) all'interno del fliuido in cui mi sto muovendo. Inoltre
Le masse delle molecole che urto devono essere piccole rispetto all'oggetto che viaggia al loro interno,
 altrimenti non vale la legge di Stokes.\\

Se gli urti non avvengono a grandi distanze di tempo possiamo eguagliare per trovare la velocità massima,
dunque basta far entrare la particella nel foro già raggiunta tale velocità (spruzzando a $\simeq 3/4mm$ dal foro).
\subsubsection{Effetto di Archimede}
Inoltre vi è la forza di Archimede da considerare, che è $\propto \rho_{aria} V_{sfera} g $
\[
m_{\sigma}=\rho_{\sigma} V_{\sigma} \quad m'=\rho_l V_{\sigma}
\]
L'equazione diviene 
\[
F_{tot}=-mg + F_{stokes} + F_{arch} = -mg + 6\pi \eta  R \vec{v} + m'g
\]
ovvero (essendo $V =  \frac{4}{3}\pi R^3 $):
\[
F_{tot}= -mg + 6\pi \eta  R v_\infty - (\rho_{\sigma}-\rho_l) \frac{4}{3}\pi R^3 g
\]
La velocità la conosciamo misurando in quanto tempo la gocciolina attraversa le tacchette, l'incognita rimanente è 
il raggio della gocciolina, ponendo a 0:
\[
R= \sqrt{\frac{6\pi \eta  v_\infty}{\frac{4}{3}\pi g + (\rho_{\sigma}-\rho_l) g} }= \sqrt{\frac{9\pi \eta }{2(\rho_{\sigma}-\rho_l) g}}
\]


\subsubsection{Osservazioni}
Ai tempi di Millikan c'era già un'idea del valore della carica elementare e la misura di Farady del rapporto carica/massa.
\begin{itemize}
\item La gocciolina può neutralizzarsi mentre cade, a causa degli urti con le particelle.
\item Per fare una misura corretta, bisogna caricare le goccioline poco, in modo da diminuire l'errore relativo.
\item Se, invece, ha ancora carica, posso sperare di governarne il movimento tramite il condensatore, cambiando la polarità del sistema
\item Se la gocciolina rallenta quando cambio la polarità, allora essa viene rallentata dal campo elettrico incontrato, potendone ricavare 
la velocità 
\item accendendo il campo elettrico, posso anche rallentare la gocciolina e addirittura riportarla in altro. 
\end{itemize}

\subsubsection{Errori}
\begin{itemize}
\item L'errore maggiore è causata dallo sbilanciamento termico che causa correnti convettive , generate dalla luce che scalda la zona in cui irragia,  che sposteranno la gocciolina.
Per ridurne l'effetto si deve far si che la finestrella d'ingresso della luce sia lontana dal punto di osservazione. \\
\item Depressioni improvvise nella stnza creano correnti d'aria molto intense, per la gocciolina, che di conseguenza avrà un modo vario.
\item non parallelità del campo elettrico, ovvero delle armature del condensatore, quasi impossibile da evitare. Per mitigare questo effetto misurerò anche la velocità
di risalita. Riscrivendo l'equazione ricordando che la gocciolina si muove anche a causa del campo elettrico, da cui ottengo due $v_\infty$.
In tal modo il termine d'attrito non dipende da come e' stato fatto l'esperimento, ma solo dalle geometria della particella, eguagliando il termine:
\[
6 \pi \eta R
\]
nelle due misure (salita e discesa).
\item Radioattività dell'ambiente esterno (anche banane), la quale può improvvisamente rallentare o velocizzare il moto della gocciolina.
\item Condizioni ambientali: potremmo avere molto attrito (giornata secca), con una gocciolina molto carica. Si consiglia di scegliere giornate intermedie per la misura. 
\end{itemize}

Allora nel momento in cui riesco a bloccare la gocciolina all'interno della scatola, allora sono nella condizione di trovare la carica della gocciolina quando le forze si bilanciano. 
\[
 -  \frac{4}{3}\pi R^3  \rho_\sigma g +   \frac{4}{3}\pi R^3 \rho_l + Q \underbrace{\frac{V}{d}}_{\text{campo elettrico}}=0
\]
da cui si ricava facilemnte $Q$.

Se da tutte le cariche noto che esiste un m.c.m. sui valori trovati, posso osservare che la carica sia quantizzata, sperando di misurare una gocciolina caricata
con un multiplo dispari della caricha elementare (altrimenti posso sperare di misurare il doppio della carica elementare). 

\subsection{Organizzazzione dei dati}
Ci aspettiamo che in un grafico: numero delle cariche avente un multiplo della carica elementare vs Q, dei picchi sui valori di Q che sono multipli della carica elementare,
con distanza fra i picchi di 1 carica elementare, che ci viene fornita dal primo intervallo. Prendo dunque il valor medio fra le distanze dei picchi per valutare la carica elementare.
Rimembrando che qundo vado a cariche elevate avrò picchi più larghi, per avere delle misure ideali, dovrò svolgere molte misure. 

Grande fattore di errore era la legge di Stokes che si utilizzava, dunque ha dovuto correggere la legge di Stokes, come:
\[
\vec{F}_a = \frac{-mg + 6\pi \eta  R \vec{v}}{1 + \alpha \frac{\lambda}{R}}
\]
il cammino libero medio è $\lambda$. 
