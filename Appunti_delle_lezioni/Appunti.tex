...
\begin{tikzpicture}[every node/.style={font=\small}, thick]

% ---------- Left diagram ----------
\begin{scope}[xshift=-6.5cm]
  % Nodes of the diamond
  \coordinate (T) at (0,2);   % top
  \coordinate (R) at (2,0);   % right
  \coordinate (B) at (0,-2);  % bottom
  \coordinate (L) at (-2,0);  % left

  % resistor in the center (between left and right)
  \draw (L) -- ++(0.6,0) to[R, l=R, -*] ++(1.4,0) -- (R);

  % diodes forming the bridge (use diode orientation arrow -> current direction)
  \draw (T) to[diode, -*, l_=] (R); % top-right diagonal
  \draw (T) to[diode, *-, l_] (L);  % top-left diagonal

  \draw (B) to[diode, *-, l_] (R);  % bottom-right diagonal
  \draw (B) to[diode, -*, l_=] (L); % bottom-left diagonal

  % input terminals and labels (Vin across top and bottom)
  \draw[|-|] (-3.6,2.1) -- ++(0, -4.2) node[midway, left] {${V_{in}}$};
  \draw[thick, ->] (-2.6,2.2) -- ++(1.8,0) node[above,pos=0.95] {I}; % top current arrow

  % output terminals (Vout across left-bottom to right-bottom)
  \draw[|-|] (-2.6,-2.6) -- ++(4.6,0) node[midway, below] {${V_{out}}$};

  % signs at resistor sides
  \node[left] at ( -0.2,0) {$-$};
  \node[right] at ( 1.2,0) {$+$};

  % small current arrows on diagonals (to resemble original)
  \draw[thick, -{Stealth[]}] (1.0,1.0) ++(-0.25,-0.25) -- ++(0.6,0.6);
  \draw[thick, -{Stealth[]}] (-1.0,1.0) ++(0.25,-0.25) -- ++(-0.6,0.6);
  \draw[thick, -{Stealth[]}] (-1.0,-1.0) ++(-0.25,0.25) -- ++(-0.6,-0.6);
  \draw[thick, -{Stealth[]}] (1.0,-1.0) ++(0.25,0.25) -- ++(0.6,-0.6);

  % title (optional)
  %\node at (0,-3.6) {Figura (a)};
\end{scope}

% ---------- Right diagram (mirrored / alternate orientation) ----------
\begin{scope}[xshift=6.5cm]
  % Nodes of the diamond
  \coordinate (T) at (0,2);   % top
  \coordinate (R) at (2,0);   % right
  ...
  % diodes: mirrored orientation (so current paths differ)
  \draw (T) to[diode, *-, l_] (R); % top-right diagonal (opposite orientation)
  \draw (T) to[diode, -*, l_=] (L);  % top-left diagonal
  ...
  % input terminals and labels (Vin across top and bottom)
  \draw[|-|] (3.6,2.1) -- ++(0, -4.2) node[midway, right] {${V_{in}}$};
  \draw[thick, <-] (0.6,2.2) -- ++(1.8,0) node[above,pos=0.05] {I}; % top current arrow (pointing left)

  % output terminals (Vout across left-bottom to right-bottom)
  \draw[|-|] (-1.8,-2.6) -- ++(4.6,0) node[midway, below] {${V_{out}}$};
  ...
  % small current arrows on diagonals (to resemble original)
  \draw[thick, -{Stealth[]}] (0.6,1.0) ++(0.25,-0.25) -- ++(-0.6,0.6);
  \draw[thick, -{Stealth[]}] (-0.6,1.0) ++(-0.25,-0.25) -- ++(0.6,0.6);
  \draw[thick, -{Stealth[]}] (-0.6,-1.0) ++(0.25,0.25) -- ++(0.6,-0.6);
  \draw[thick, -{Stealth[]}] (0.6,-1.0) ++(-0.25,0.25) -- ++(-0.6,-0.6);

  % title (optional)
  %\node at (0,-3.6) {Figura (b)};
\end{scope}
